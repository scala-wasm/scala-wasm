...
private class FunctionEmitter private (
    ...
    (generatedType, expectedType) match {
      ...
      case (primType: PrimTypeWithRef, _) =>
        ...
        primType match {
          case NullType =>
            expectedType match {
              case ClassType(BoxedStringClass, true) if ctx.hasJSInterop =>
        ...
        array.tpe match {
          case ArrayType(arrayTypeRef, _) =>
            ...
            def genRhs(): Unit = {
              genTree(rhs, lhs.tpe)
              lhs.tpe match {
                case ClassType(BoxedStringClass, _) if ctx.hasJSInterop =>
  ...
  private def genLiteral(tree: Literal, expectedType: Type): Type = {
    if (expectedType == VoidType) {
      ...
      expectedType
        ...
        expectedType == ClassType(BoxedStringClass, true)) {
  ...
  private def genTryCatchWithEH(tree: TryCatch, expectedType: Type): Type = {
    ...
    val exceptionType =
      if (ctx.hasJSInterop) watpe.RefType.externref
      else watpe.RefType.anyref
  ...
  private def genJSClosure(tree: Closure): Type = {
    ...
    assert(ctx.hasJSInterop, s"Unexpected JS Closure at $pos")
