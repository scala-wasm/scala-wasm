...
import org.scalajs.linker.backend.webassembly.Identitities.FunctionID
...
object WasmTransients {
  ...
  final case class WasmAllocate(size: Tree) extends Transient.Value {

    val tpe: Type = ClassType(ClassName("java.util.internal.wasm.MemorySegment"), nullable = false)
    override def traverse(traverser: Traverser): Unit = traverser.traverse(size)

    override def transform(transformer: Transformer, isStat: Boolean)(implicit pos: Position): Tree =
      Transient(WasmAllocate(transformer.transformExpr(size)))

    override def printIR(out: IRTreePrinter): Unit = {
      out.print("$allocate")
      out.printArgs(List(size))
    }
  }

  final case class WasmFree() extends Transient.Value {
  ...
  final case class WasmLoad(loadType: WasmType.SimpleType, offset: Tree) extends Transient.Value {

    val tpe: Type = WasmType.resultTypeOf(loadType)

    override def traverse(traverser: Traverser): Unit =
      traverser.traverse(offset)

    override def transform(transformer: Transformer, isStat: Boolean)(implicit pos: Position): Tree =
      Transient(WasmLoad(loadType, transformer.transformExpr(offset)))
  ...
  final case class WasmStore(storeType: WasmType.SimpleType, offset: Tree, value: Tree) extends Transient.Value {
    val tpe: Type = NoType
    ...
    override def transform(transformer: Transformer, isStat: Boolean)(implicit pos: Position): Tree =
      Transient(WasmStore(storeType, transformer.transformExpr(offset), transformer.transformExpr(value)))
  ...
  final case class WasmFunctionCall(function: FunctionID, args: List[Tree], val tpe: Type)
      extends Transient.Value {

    override def traverse(traverser: Traverser): Unit =
      args.foreach(traverser.traverse)

    override def transform(transformer: Transformer, isStat: Boolean)(implicit pos: Position): Tree =
      Transient(WasmFunctionCall(function, args.map(transformer.transformExpr), tpe))
  ...
  object WasmType {
    ...
    final case object I8 extends SimpleType
    final case object I32 extends SimpleType

    def resultTypeOf(tpe: SimpleType): Type = tpe match {
      case I8  => ByteType
      case I32 => IntType
    }
  }
