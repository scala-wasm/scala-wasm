...
final class CoreWasmLib(coreSpec: CoreSpec) {
  ...
  private def genPrimitiveAsInstance(primType: PrimType)(implicit ctx: WasmContext): Unit = {
    genPrimitiveOrBoxedClassAsInstance(primType, targetTpe = primType, isUnbox = true)
  }

  /** `as.boxedClass: anyref -> (ref null bloxedClass)`. */
  private def genBoxedClassAsInstance(primType: PrimType)(implicit ctx: WasmContext): Unit = {
    val targetTpe = ClassType(PrimTypeToBoxedClass(primType), nullable = true)
    genPrimitiveOrBoxedClassAsInstance(primType, targetTpe, isUnbox = false)
  }

  /** Common logic for `genPrimitiveAsInstance` and `genBoxedClassAsInstance`. */
