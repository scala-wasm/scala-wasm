...
final class CoreWasmLib(coreSpec: CoreSpec, globalInfo: LinkedGlobalInfo) {
  ...
  val stringType = if (true /*isWASI*/) RefType(genTypeID.i16Array) else RefType.extern // scalastyle:ignore
  val nullableStringType =
    if (true /*isWASI*/) RefType.nullable(genTypeID.i16Array) // scalastyle:ignore
  ...
  private def genHelperImports()(implicit ctx: WasmContext): Unit = {
    ...
    if (false /*!isWASI*/) { // scalastyle:ignore
    ...
    } else {
      {
        ...
        fb.block(RefType.anyref) { nonEqRef =>
          fb += LocalGet(a)
          fb += BrOnCastFail(nonEqRef, RefType.anyref, RefType.eqref)
  ...
  private def genArrayTypeData()(implicit ctx: WasmContext): Unit = {
    ...
    fb.loop() { loopLabel =>
      fb.block(objectVTableType) { arrayOfIsNonNullLabel =>
        ...
        fb += (if (true /*isWASI*/) RefNull(HeapType(genTypeID.i16Array)) // scalastyle:ignore
  ...
  private def genIsInstance()(implicit ctx: WasmContext): Unit = {
    ...
    fb.switch(Int32) { () =>
      ...
      List(KindJSType, KindJSTypeWithSuperClass) -> { () =>
        if (true /*isWASi*/) { // scalastyle:ignore
          fb += I32Const(0) // shouldn't reach here
        } else {
